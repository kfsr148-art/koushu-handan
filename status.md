...
**手待ち** — 三つを一度に当てる綴りへまとめる

未処理 0 件 ／ 2026-09-21 23:34:09 書き出し（VAIOの時計）
...
実機: 管理者の PowerShell で走らせたあと、dumps\apply-result.txt に「① SysMain を止めた」「② pagefile を 4096/4096 にした」「③ VAIO Care の予定を無効にした」が並び、確かめの行が SysMain : Stopped / Disabled と VAIO Care : Disabled になること。pagefile だけは再起動のあとに 初期4096 最大4096 になる。
...
### 30. ✅ 終わりました（返事不要）：未了4項目の一覧／枠の上限の結果／使用量の鈴の丸め
...
### 32. ✅ 終わりました（返事不要）：雲へ重い仕事を回す道を一本作る
...
### 35. 🔎 調べました：--continue の枠は重複。確かめ直して台帳を整えた
...
### 39. ✅ 終わりました（返事不要）：立てる数を減らす／外へ出る子を減らす／--continue を外す
