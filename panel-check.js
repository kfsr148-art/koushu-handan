  ...
  for (const k of Object.keys(ST)) {
    ...
    if (WANT[k]) {
      ...
      if (r.stText && /作業中|次の指示待ち|ヨシを返して/.test(r.stText)) {
        ...
        /* いまのパネルは**異常を画面に出さない**（2026-09-01 に実測して分かった）。
             ・黄色の行（stateLineText）は 作業中／次の指示待ち／ヨシ待ち の三通りだけ
             ・異常の札は #list にあるが、v97 で一覧ごと畳んである
             ・状態の札（#state）は v102 で畳んだ
           **どう見せるかは裁定待ち**なので、落ちには数えず、毎回ここに出して忘れないようにする。 */
        note('異常：いまのパネルは画面に出さない（黄色の行は三通りだけ・札は畳んだ一覧の中）。**裁定待ち**');
        skipped++;
      }
    }
  }
