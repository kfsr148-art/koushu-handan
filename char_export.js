...
/* 主人公の素材1枚の差し替えを一本で行う。キャラが替わっても使える名前にしてある。
   ...
   ★焼き直すときの注意（v1399 の時点）
   いま本体に入っている斬撃の弧（三日月）は、旧法——明るさをそのまま不透明度にする作り方——で
   焼いてあるので薄い。この道具は白抜きを作り替えてあり、人物の内側を不透明のまま残すため、
   同じ元絵から焼き直すと弧が濃く出る。指示していない見え方の変化になるので、
   弧を持つコマ（airSlash・u/ur/r/dr/d/dl/l/ul など）を焼き直したら、
   必ず本番と同じ暗い背景 #0d1b16 に新旧を並べて見比べること。焼き直した当日に気づく。 */
'use strict';
